...
export async function ListProducts(
  { browserPage, timeout }: ListProductsDependencies,
  { page }: ListProductsPayload
): Promise<ListProductsResult> {
  // Lista de arcillas
  await browserPage.goto(`https://www.marphil.com/tienda/${page}/`, {
    timeout: 0,
  });
  ...
  return await browserPage.evaluate(
    async ({ timeout }): Promise<ListProductsResult> => {
